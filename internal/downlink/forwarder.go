...
// SourceIdentity identifies one publisher attachment. Generation is assigned
// by AudioBus and changes on reconnect even if the publisher reuses its SSRC.
...
func (t *rtpForwardTranslator) translate(pkt *rtp.Packet, source SourceIdentity) (reset bool) {
	return t.translateAt(pkt, source, time.Time{})
}

func (t *rtpForwardTranslator) translateAt(pkt *rtp.Packet, source SourceIdentity, arrival time.Time) (reset bool) {
	if !t.initialized {
		t.initialized = true
		t.source = source
