// Package ratelimit provides a per-IP sliding-window rate limiter for room
// creation. It is intentionally simple: one sync.Map keyed by IP string,
// each holding a *ipState with an atomic counter and a window-start timestamp.
// A background goroutine sweeps stale entries every cleanupInterval.
//
// The limiter is not persistent and resets on process restart, which is
// acceptable for the relay's Phase 2 requirements.
package ratelimit
