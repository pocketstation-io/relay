...
//
// Downlink satisfies the session.BusSubscription interface so it can be stored
...
func NewForwardingDownlink(id string, inner session.PacketWriter, feedback *FeedbackReader) *Downlink {
...
// unit tests).
func NewPassthrough(id string, inner session.PacketWriter) *Downlink {
...
// WriteRTP satisfies session.BusSubscription.
//
...
func (d *Downlink) WriteRTPWithCaptureTime(pkt *rtp.Packet, captureTime time.Time, known bool) error {
	return d.WriteRTPWithSource(pkt, captureTime, known, session.SourceIdentity{})
}
...
// subscriber. It must be called before the downlink is published to the session.
...
// StopForwarding terminates the serialized forwarding worker without waiting for
// RTCP. The feedback reader exits when its PeerConnection closes.
func (d *Downlink) StopForwarding() {
	d.pacer.Stop()
}
...
// BusID is left empty here; RelaySession fills it from the subscription entry.
// All reads are atomic; safe to call from any goroutine.
func (d *Downlink) Snapshot() session.SubscriptionSnapshot {
	pacer := d.pacer.Snapshot()
	snapshot := session.SubscriptionSnapshot{
		SubscriberID:                      d.id,
		Mode:                              pacer.Mode,
