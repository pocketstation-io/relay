...
func Encode(primaryPayloadType uint8, primary []byte, redundant []Block) ([]byte, error) {
	...
	for i := range redundant {
		...
		if b.TimestampOffsetSamples > maxTimestampOffset {
			return nil, fmt.Errorf("red: redundant block %d timestamp offset %d exceeds %d", i, b.TimestampOffsetSamples, maxTimestampOffset)
		}
	...
	for i := range redundant {
		...
		v := (b.TimestampOffsetSamples << 10) | uint32(len(b.Payload))
...
type Parsed struct {
	Redundant      []Block
	PrimaryType    uint8
	PrimaryPayload []byte // remainder after all block headers and redundant payloads
}
...
func Parse(payload []byte) (Parsed, error) {
	...
	for {
		...
		redHdrs = append(redHdrs, hdr{pt: pt, tsOff: tsOffSamples, length: length})
	...
	for _, h := range redHdrs {
		...
		p.Redundant = append(p.Redundant, Block{
			PayloadType:            h.pt,
			TimestampOffsetSamples: h.tsOff,
