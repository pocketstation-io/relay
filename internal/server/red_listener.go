...
// redListener decorates a packet writer with RFC 2198 redundancy.
...
// session.AudioBus.forwardLoop stays codec-agnostic — it just calls WriteRTP; the RED framing
// lives here.
type redListener struct {
	inner                session.PacketWriter // underlying audio/red TrackLocalStaticRTP
...
func newREDListener(inner session.PacketWriter, opusPT uint8) *redListener {
