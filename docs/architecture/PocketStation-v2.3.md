...
| Opus frame duration left implicit (`960 samples at 48kHz`) | RELAY-012 added: 20ms default; 10ms option for voice-agent mode after benchmarks | Frame duration cascades into pool sizing, packet rate, jitter buffer, CPU, bitrate overhead — must be explicit |
| Internal sample format + channel layout policy unstated | RELAY-013 added: interleaved f32 internal; mono 48kHz voice mode; stereo 48kHz music/broadcast; `AudioProcessorNode` declares accepted channel layout, graph auto-inserts `MonoMixNode` | Every encoder, VAD, STT, and ML node makes assumptions; lock the policy or pay for surprises |
...
| Opus frame duration implicit | RELAY-012 makes it explicit |
| Channel layout / sample format policy implicit | RELAY-013 makes it explicit |
...
pub struct AudioFrame {
    ...
    pub sample_rate:     u32,         // 48_000 internally (RELAY-013)
    pub channels:        u8,          // 1 voice / 2 music (RELAY-013)
    pub format:          SampleFormat, // F32LE interleaved (RELAY-013)
...
/// At 20ms frame duration (RELAY-012): 64 × 20ms = 1.28s of headroom.
...
pub struct AudioBufferPool {
    ...
    slot_size: usize,               // samples per slot — 960 at 48kHz/20ms (RELAY-012)
...
pub trait AudioProcessorNode: Send {
    ...
    /// or appropriate adapter when upstream layout differs. See RELAY-013.
...
### RELAY-001: FFI/JNI Boundary Ownership
...
### RELAY-002: Star Topology — No Relay Chains
...
### RELAY-003: Custom Go Relay — No LiveKit
...
**Channel layout rule (RELAY-013):** ML nodes declare `accepted_channels()`. Most voice ML expects mono. The graph inserts a `MonoMixNode` upstream of any mono-only node when fed stereo, transparently and at the cost of one extra allocation-free pass.
...
let station = PocketStation::builder()
    ...
    .room_id("abc123")
    .mode(AudioMode::Voice)                    // mono 48kHz internal (RELAY-013)
    .opus_frame_duration_ms(20)                // default; 10 for voice-agent (RELAY-012)
...
AudioFrame with pool handles, interleaved f32 48kHz (RELAY-013)
SPSC ring buffer (proptest-verified invariants; crate choice per §26.8)
FrameBus
ProcessorGraph (empty graph, PassthroughNode) with accepted_channels routing (RELAY-013)
ClockSync (algorithm decision per §26.3)
Opus encoder/decoder at 20ms default frame duration (RELAY-012)
Fake sine-wave source
File output sink
Full metrics
Backpressure policy decided (RELAY-004, §26.1)
```
...
RELAY-004 through RELAY-013 written and merged.
```
...
Mitigation:
- RELAY-001 defines the complete boundary contract before code
...
**Recommended:** A. Documented as RELAY-004.
...
**Recommended:** B for voice, hook for C in music-mode. Documented as RELAY-006.
...
**Recommended:** B with explicit `negotiated: NegotiatedCapability` on the stream. Documented as RELAY-007.
...
Tooling: `cargo-release` with sequenced publish and per-crate retry. Git tag strategy: single `v0.X.Y` at workspace root, all crates published at the same version. Documented as RELAY-008.
...
"No re-encode" does not mean "no allocation." Documented as RELAY-009.
...
**Recommended:** B for Phase 1, optional swap to C in Phase 5. Documented as RELAY-010.
...
**Default candidate:** `rtrb` — fixed-capacity allocation at construction, no allocation afterward, lock-free wait-free reads/writes. Verified against criteria; confirm in Phase 0 prototype. Documented as RELAY-011.
...
**Recommended:** 20ms default. Make it a runtime parameter on `PocketStation::builder()`. After Phase 1 latency measurements, enable 10ms for voice-agent mode if benchmarks justify the CPU/overhead tradeoff. Documented as RELAY-012.
...
**`AudioProcessorNode::accepted_channels()`** lets each node declare what it can ingest. Graph builder checks the chain and inserts adapter nodes (`MonoMixNode`, `StereoBroadcastNode`) at the right positions. Insertions are zero-allocation passes using the existing pool. Documented as RELAY-013.

---

*Document version 2.3 — green-light version. No further structural rewrites planned. Phase 0 begins when RELAY-004 through RELAY-013 are written and merged.*
